...
ns = api.namespace('fitting_comparisonrules', path='/fitting/comparisonrules', description='Operations related to Comparison rule')
...
@ns.route('/<string:id>')
class ComparisonRuleItem(Resource):
    @api.marshal_with(comparison_rule)
    def delete(self, id):
        """
        Api method to delete rule.
        """
        _compRuleRep.delete({'@rid': id})
        return None, 204
